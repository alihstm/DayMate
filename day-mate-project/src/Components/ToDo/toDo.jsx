...
const ToDo = () => {
  ...
  const handleAddTag = (tag, isRemove = false) => {
    setNewTaskState((prev) => ({
      ...prev,
      tags: isRemove ? prev.tags.filter((t) => t !== tag) : [tag, ...prev.tags],
    }));
  };

  return (
    <section className="flex flex-col items-center justify-between px-3 py-4 sm:w-[26%] w-[22rem] sm:h-[100%] h-[45%] rounded-2xl custom-whiteLess-bg">
      <TaskHeader
        ...
        <div className="flex flex-col items-center justify-between w-[82%] h-[100%]">
          ...
          >
            ...
            <div className="flex flex-row items-center justify-between px-2 w-full mt-2">
              ...
              <button className="flex flex-row items-center justify-between text-sm w-15 transition hover:cursor-pointer text-gray-700 hover:text-black">
